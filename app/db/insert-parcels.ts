 ...
 *
 * ## Capacity locking contract
 *
 * Callers that enforce parcel capacity must lock every affected pickup-location
 * row inside their transaction before reading counts or limits. The shared
 * `lockPickupLocationsForCapacity` helper provides deterministic row locking.
 * Keeping validation and this insert in that same transaction prevents two
 * concurrent requests from both claiming the final available place.
 *
