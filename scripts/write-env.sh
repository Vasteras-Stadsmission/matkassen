...
write_env_file() {
...

    {
        ...
        # DATABASE_SSL is optional — only emit when set so unset/empty
        # means "defer to DATABASE_URL" (the default for the trusted
        # Docker network). instrumentation.ts validates the value at
        # boot, so a typo like "required" fails fast in staging.
