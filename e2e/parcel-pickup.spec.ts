...
/**
 * Parcel Handout Feature Tests (Smoke Tests)
 *
 ...
 * - ❌ Do NOT actually mark parcels as collected (would affect database)
 * - ❌ Do NOT assume specific data exists
 *
 * These tests verify the critical bug fixes:
 * - Finding #1: POST/PATCH method mismatch (fixed)
 * - Finding #2: Refresh after collection (fixed)
 */

test.describe("Parcel Handout Feature (Smoke Tests)", () => {
    test("parcel collection API endpoint exists and requires auth", async ({ request }) => {
        ...
        const response = await request.patch("/api/admin/parcel/test-id/collect");
    ...
    test("documents expected parcel status badge behavior", async () => {
        ...
        // - Green badge = "collected" - parcel was successfully collected
        // - Red badge = "not collected" - parcel from PREVIOUS day that was never collected
        // - Gray badge = "cancelled" - parcel was soft-deleted
        //
        // IMPORTANT: Same-day parcels ALWAYS show as "upcoming" (blue badge)
        // even if the handout window has passed, because households may arrive
        // throughout the day and we don't want to prematurely show "not collected"
