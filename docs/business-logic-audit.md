...
| Household management                          | Household actions, eligibility, member/contact fields, deletion/archive behavior                  | In progress | Create/edit share several guards for active options, responsible staff, and normalized contact data. Removal/anonymization preserves history while deleting free-text PII.                    |
...
| 2026-06-18 | Household management           | Behavior ambiguity | Active household phone numbers are protected by the partial unique index `idx_households_phone_unique`. Edit also preflights duplicate active phone numbers and returns `DUPLICATE_PHONE`, but enrollment relies on the advisory duplicate checker plus the database constraint, so a bypass or race would fall into the generic enrollment error path instead of a localized duplicate-phone result.     | Add a server-side duplicate-phone guard to `enrollHousehold()` that mirrors edit behavior, then cover the create/edit duplicate-phone contract with integration tests.                           | Open      |
| 2026-06-18 | Household management           | Duplicated rule    | Enrollment and edit both validate selectable household options for dietary restrictions, pets, and additional needs. Edit intentionally allows already-linked inactive options while blocking newly added inactive options; enrollment blocks inactive options outright.                                                                                                                                  | Keep the rule split explicit for now. If another option family changes, extract a small shared helper that makes the create-vs-edit policy an argument instead of duplicating each option query. | Accepted  |
| 2026-06-18 | Household management           | Over-abstraction   | Deletion-first call-site checks did not find a clean household-management deletion. `HouseholdWizard.mode` has real create and edit production callers, and `FoodParcelsForm` is also used by parcel management. `ParcelCard` has one production caller through `ParcelList`, but it owns parcel-status badge rendering and belongs with the parcel-status UI cleanup already scoped elsewhere.           | Leave these APIs in place for this household audit. Revisit `ParcelCard` only during a parcel-status/UI simplification pass.                                                                     | Accepted  |
| 2026-06-18 | Household management           | Test gap           | Responsible staff assignment has several business rules: enrollment requires a responsible staff user, can default to the current staff user, and blocks assignment to deactivated users except when preserving the existing responsible user during edit. The rules are enforced in code, but there is no focused household integration test for the deactivated-user branch or the enrollment fallback. | Add focused integration coverage before changing responsible-staff assignment behavior.                                                                                                          | Open      |
| 2026-06-18 | Household management           | Test gap           | Household removal/anonymization rules are documented in code and covered by integration tests for audit pruning, hard delete without service history, anonymization with history, option unlinking, verification-note deletion, and date-only removal blocking.                                                                                                                                           | Treat removal/anonymization as covered for now; revisit only when adding new household-linked tables that can contain PII.                                                                       | Accepted  |
...
| 2026-06-18 | Do not merge household option guards blindly.                 | Create and edit intentionally differ for inactive options: creation should only accept active choices, while edit may preserve already-linked inactive choices. |
...
| 2026-06-18 | Household management           | Audited household create/edit/remove rules, ran the deletion-first call-site checks, and recorded follow-up findings for duplicate phone handling, option availability guards, responsible staff assignment, and anonymization coverage.      | Not run; documentation-only audit update.                                                                                                                                                                                                                  |
