...
class CacheHandler {
    ...
    async get(key) {
        ...
        try {
            ...
            if (isStale) {
                console.log(
                    chalk.yellow(
                        `⚠️ Cache entry for key ${chalk.bold(key)} is stale due to stale tags`,
                    ),
                );
                return null;
            }
    ...
    async revalidateTag(tags) {
        ...
        console.log(chalk.magenta(`🔄 Revalidating tags: ${chalk.bold(tagsArray.join(", "))}`));
        ...
        for (const tag of tagsArray) {
            ...
            console.log(
                chalk.blue(
                    `   ⏰ Tag ${chalk.bold(tag)} revalidated at ${new Date(now).toISOString()}`,
                ),
            );
        }
        ...
        console.log(
            chalk.magenta(
                `✨ Revalidation complete for tags: ${chalk.bold(tagsArray.join(", "))}.`,
            ),
        );
    }
}
